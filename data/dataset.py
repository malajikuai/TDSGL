...
class Dataset(object):
    ...
    def _load_data(self, config):
        ...
        #使用一阶连通性作为类别关系
        u2u = self.train_matrix @ self.train_matrix.transpose()
        i2i = self.train_matrix.transpose() @ self.train_matrix
        u2u_dense, i2i_dense = u2u.todense(), i2i.todense()
        u2u_dense[u2u_dense < 3], i2i_dense[i2i_dense < 3] = 0.0, 0.0
        u2u_dense[u2u_dense > 3], i2i_dense[i2i_dense > 3] = 1.0, 1.0
        self.u2u_dok = sparse.dok_matrix(u2u_dense)
        self.i2i_dok = sparse.dok_matrix(i2i_dense)
        self.u2u_dense = u2u_dense
        self.i2i_dense = i2i_dense

        # self.user_list = [[] for _ in range(self.num_users)]
        # self.item_list = [[] for _ in range(self.num_items)]
        # temp_user = np.stack([self.u2u_coo.row, self.u2u_coo.col], axis=0).transpose()
        # temp_item = np.stack([self.i2i_coo.row, self.i2i_coo.col], axis=0).transpose()
        # self.process(temp_user, 'user'), self.process(temp_item, 'item')
        # self.u2u = self.csr2tf(csr_matrix(u2u.toarray()))
        # self.i2i = self.csr2tf(csr_matrix(i2i.toarray()))
    ...
    def process(self, temp, flag):
        if flag == 'user':
            for i in temp:
                self.user_list[i[0]].append(i[1])
        else:
            for i in temp:
                self.item_list[i[0]].append(i[1])
    ...
    def get_train_interactions_v1(self, dok_matrix):
