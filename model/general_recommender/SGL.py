...
class SGL(AbstractRecommender):
    def __init__(self, sess, dataset, conf):
        ...
        # self.u2u = self.csr2tf(self.dataset.u2u)
        # self.i2i = self.csr2tf(self.dataset.i2i)
        # self.u2u = np.array(self.dataset.user_list)
        # self.i2i = np.array(self.dataset.item_list)
        # self.u2u_dok, self.i2i_dok = self.dataset.u2u_dok, self.dataset.i2i_dok

        # self.users_transform, self.items_transform = self.transform(3, 64, 0.1), self.transform(3, 64, 0.1)

        self.u2u_dense, self.i2i_dense = self.dataset.u2u_dense, self.dataset.i2i_dense

        #这句话有作用?
        #2022-08-02 18:54:24.101: epoch 18:	0.03008423  	0.06703205  	0.05516584  	0.10122290  	0.11343800
        #2022-08-02 19:34:56.042: epoch 18:	0.03023103  	0.06769536  	0.05523276  	0.10014712  	0.11289060
        ...
        user1, user2 = self.dataset.get_train_interactions_v1(self.dataset.u2u_dok)
        item1, item2 = self.dataset.get_train_interactions_v1(self.dataset.i2i_dok)
        self.users_norm_adj = self.get_label_matrix(user1, user2, self.n_users)
        self.items_norm_adj = self.get_label_matrix(item1, item2, self.n_items)

        # self.u2u_norm_adj = self.create_adj_mat(())
        ...
        #2022-08-02 18:14:04.341: epoch 18:	0.03023261  	0.06770167  	0.05523601  	0.10015239  	0.11289545 阈值：0.2-0.95
    ...
    def transform(self, net, embedding_dim, dropout):
        model = tf.keras.models.Sequential()
        for i in range(net):
            if i == 0:
                model.add(tf.keras.layers.Dense(64, input_shape=(64,), activation='relu',  kernel_initializer='glorot_uniform'))
    ...
    def csr2tf(self, csr_mat):
        Acoo = csr_mat.tocoo()
        indices, values = np.stack([Acoo.row, Acoo.col], axis=0).transpose(), np.ones_like(Acoo.data, dtype=np.float32)
        # sp_tf = tf.sparse.reorder(tf.sparse.SparseTensor(indices, values, csr_mat.shape))
        sp_tf = tf.sparse.SparseTensor(indices, values, csr_mat.shape)
        return tf.sparse_tensor_to_dense(sp_tf)

    def get_label_matrix(self, node1, node2, n_nodes):
        user_np = np.array(node1)
        item_np = np.array(node2)
        ratings = np.ones_like(user_np, dtype=np.float32)
        adj_mat = sp.csr_matrix((ratings, (user_np, item_np)), shape=(n_nodes, n_nodes))
        # pre adjcency matrix
    ...
    def _create_variable(self):
        with tf.name_scope("input_data"):
            ...
            self.u2u_label = tf.placeholder(tf.float32, shape=(None, self.n_users))
            self.i2i_label = tf.placeholder(tf.float32, shape=(None, self.n_items))
        ...
        with tf.name_scope("embedding_init"):
            ...
            self.users_transform = tf.Variable(initializer([self.embedding_size, self.embedding_size]),
                                               name='users_transform')
            self.items_transform = tf.Variable(initializer([self.embedding_size, self.embedding_size]),
                                               name='items_transform')

    def build_graph(self):
        self._create_variable()
        with tf.name_scope("inference"):
            self.ua_embeddings, self.ia_embeddings, self.ua_embeddings_sub1, self.ia_embeddings_sub1, self.ua_embeddings_sub2, self.ia_embeddings_sub2, self.user_tmp, self.item_tmp = self._create_lightgcn_SSL_embed()
        ...
        with tf.name_scope("loss"):
            if self.pretrain:
                ...
                if self.ssl_mode in ['user_side', 'item_side', 'both_side']:
                    ...
                    self.ssl_loss = self.calc_ssl_loss_adj()
    ...
    def _create_lightgcn_SSL_embed(self):
        ...
        user_tmp, item_tmp = tf.split(all_embeddings[2], [self.n_users, self.n_items], 0)
        ...
        return u_g_embeddings, i_g_embeddings, u_g_embeddings_sub1, i_g_embeddings_sub1, u_g_embeddings_sub2, i_g_embeddings_sub2, user_tmp, item_tmp
    ...
    def calc_ssl_loss_auxiliary(self):
        "计算辅助Pos_score"
        user_emb1 = tf.nn.embedding_lookup(self.weights['user_embedding'], self.users)
        user_adj_mat = self._convert_sp_mat_to_sp_tensor(self.users_norm_adj)
        local_user_emb = tf.sparse_tensor_dense_matmul(user_adj_mat, self.weights['user_embedding'])
        user_emb2 = tf.nn.embedding_lookup(local_user_emb, self.users)
        normalize_user_emb1 = tf.nn.l2_normalize(user_emb1, 1)
        normalize_user_emb2 = tf.nn.l2_normalize(user_emb2, 1)
        pos_score_user = tf.reduce_sum(tf.multiply(normalize_user_emb1, normalize_user_emb2), axis=1)
        Auxiliary_pos_user_score = tf.exp(pos_score_user / self.ssl_temp)

        item_emb1 = tf.nn.embedding_lookup(self.weights['item_embedding'], self.pos_items)
        item_adj_mat = self._convert_sp_mat_to_sp_tensor(self.items_norm_adj)
        local_item_emb = tf.sparse_tensor_dense_matmul(item_adj_mat, self.weights['item_embedding'])
        item_emb2 = tf.nn.embedding_lookup(local_item_emb, self.users)
        normalize_item_emb1 = tf.nn.l2_normalize(item_emb1, 1)
        normalize_item_emb2 = tf.nn.l2_normalize(item_emb2, 1)
        pos_score_item = tf.reduce_sum(tf.multiply(normalize_item_emb1, normalize_item_emb2), axis=1)
        Auxiliary_pos_item_score = tf.exp(pos_score_item / self.ssl_temp)

        return Auxiliary_pos_user_score, Auxiliary_pos_item_score


    def calc_ssl_loss_adj(self):
        '''
        The denominator is summing over all the user or item nodes in the whole grpah
        '''
        # local_pos_user_score, local_pos_item_score = self.calc_ssl_loss_auxiliary()
        if self.ssl_mode in ['user_side', 'both_side']:
            ...
            user_adj_mat = self._convert_sp_mat_to_sp_tensor(self.users_norm_adj)
            local_user_latent = tf.sparse_tensor_dense_matmul(user_adj_mat, self.weights['user_embedding'])
            local_user_latent = tf.nn.embedding_lookup(local_user_latent, self.users) + 0.01*(tf.nn.embedding_lookup(self.weights['user_embedding'], self.users))
            normalize_local_user_latent = tf.nn.l2_normalize(local_user_latent, 1)
            local_pos_user_score = tf.reduce_sum(tf.multiply(normalize_user_emb1, normalize_local_user_latent), axis=1)
            local_pos_user_score = tf.exp(local_pos_user_score / self.ssl_temp)

            "根据相似性计算样本之间的label关系，假设处于某个区间的为理想负样本（困难负样本）"

            # user_label = 1.0 - tf.nn.embedding_lookup(self.u2u, self.users)
            ttl_score_user = ttl_score_user - ttl_score_user * self.u2u_label

            pos_score_user = tf.exp(pos_score_user / self.ssl_temp) + 0.02 * local_pos_user_score
            ttl_score_user = tf.reduce_sum(tf.exp(ttl_score_user / self.ssl_temp), axis=1)
        ...
        if self.ssl_mode in ['item_side', 'both_side']:
            ...
            item_adj_mat = self._convert_sp_mat_to_sp_tensor(self.items_norm_adj)
            local_item_latent = tf.sparse_tensor_dense_matmul(item_adj_mat, self.weights['item_embedding'])
            local_item_latent = tf.nn.embedding_lookup(local_item_latent, self.users) + 0.01*(tf.nn.embedding_lookup(self.weights['item_embedding'], self.users))
            normalize_local_item_latent = tf.nn.l2_normalize(local_item_latent, 1)
            local_pos_item_score = tf.reduce_sum(tf.multiply(normalize_item_emb1, normalize_local_item_latent), axis=1)
            local_pos_item_score = tf.exp(local_pos_item_score / self.ssl_temp)

            "根据相似性计算样本之间的label关系，假设处于某个区间的为理想负样本（困难负样本）"
            ttl_score_item = ttl_score_item - ttl_score_item * self.i2i_label

            pos_score_item = tf.exp(pos_score_item / self.ssl_temp) + 0.02 * local_pos_item_score
            ttl_score_item = tf.reduce_sum(tf.exp(ttl_score_item / self.ssl_temp), axis=1)
    ...
    def calc_ssl_loss_v2(self):
        ...
        if self.ssl_mode in ['user_side', 'both_side']:
            ...
            "根据相似性计算样本之间的label关系，假设处于某个区间的为理想负样本（困难负样本）"
            user_label = tf.cast((ttl_score_user > 0.2) & (ttl_score_user < self.thresh), dtype=tf.float32)
            # user_label = 1.0 - tf.nn.embedding_lookup(self.u2u, self.users)
            pre_user_score = user_label * ttl_score_user
            pre_user_score = pre_user_score / tf.reduce_sum(pre_user_score, axis=1, keepdims=True)
            ttl_score_user = tf.cond(self.epoch > self.signal, lambda: pre_user_score, lambda: ttl_score_user)
        ...
        if self.ssl_mode in ['item_side', 'both_side']:
            ...
            "根据相似性计算样本之间的label关系，假设处于某个区间的为理想负样本（困难负样本）"
            item_label = tf.cast((ttl_score_item > 0.2) & (ttl_score_item < self.thresh), dtype=tf.float32)
            # item_label = 1.0 - tf.nn.embedding_lookup(self.i2i, self.pos_items)
            pre_item_score = item_label * ttl_score_item
            # pre_item_score = pre_item_score / tf.reduce_sum(pre_item_score, axis=1, keepdims=True)
            ttl_score_item = tf.cond(self.epoch > self.signal, lambda: pre_item_score, lambda:ttl_score_item)
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for bat_users, bat_pos_items, bat_neg_items in data_iter:
                "获得users和items的类类关系,只需要构建一个batch*n_users/n_items的张量(矩阵)"
                # # [[i for i in range(self.n_users)] for _ in range(len(bat_users))]
                # current_u2u, current_i2i = [self.u2u[i] for i in bat_users], [self.i2i[i] for i in bat_pos_items]
                # all_user_list, all_item_list = [i for i in range(self.n_users)], [i for i in range(self.n_items)]
                # u2u_label, i2i_label = map(lambda x, y=all_user_list: list(set(y)-set(x)), current_u2u), map(lambda x, y=all_item_list: list(set(y)-set(x)), current_i2i)
                # u2u_label, i2i_label = list(u2u_label), list(i2i_label)
                u2u_label, i2i_label = np.array(self.u2u_dense[bat_users], dtype=np.float32), np.array(self.i2i_dense[bat_pos_items], dtype=np.float32)
                feed_dict = {self.users: bat_users,
                             self.pos_items: bat_pos_items,
                             self.neg_items: bat_neg_items,
                             self.u2u_label: u2u_label,
                             self.i2i_label: i2i_label,
                             self.epoch: epoch}
                if self.aug_type in [0, 1]:
                    feed_dict.update({
